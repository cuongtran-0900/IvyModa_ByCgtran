  ...
  return (
    <div>
      <h2 id="notification-heading" data-cy="NotificationHeading">
        <Translate contentKey="ivyModaApp.notification.home.title">Notifications</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="ivyModaApp.notification.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/notification/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="ivyModaApp.notification.home.createLabel">Create new Notification</Translate>
          </Link>
        </div>
      </h2>
        ...
        {notificationList && notificationList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="ivyModaApp.notification.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="ivyModaApp.notification.order">Order</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {notificationList.map((notification, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {notification.createAt ? <TextFormat type="date" value={notification.createAt} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>{notification.isRead ? 'true' : 'false'}</td>
                  <td>{notification.order ? <Link to={`/order/${notification.order.id}`}>{notification.order.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="ivyModaApp.notification.home.notFound">No Notifications found</Translate>
            </div>
          )
        )}
      </div>
