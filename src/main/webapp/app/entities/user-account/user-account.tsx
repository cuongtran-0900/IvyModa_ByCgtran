  ...
  return (
    <div>
      <h2 id="user-account-heading" data-cy="UserAccountHeading">
        <Translate contentKey="ivyModaApp.userAccount.home.title">User Accounts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="ivyModaApp.userAccount.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/user-account/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="ivyModaApp.userAccount.home.createLabel">Create new User Account</Translate>
          </Link>
        </div>
      </h2>
        ...
        {userAccountList && userAccountList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="ivyModaApp.userAccount.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="ivyModaApp.userAccount.name">Name</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                <th className="hand" onClick={sort('email')}>
                  <Translate contentKey="ivyModaApp.userAccount.email">Email</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('email')} />
                </th>
                <th className="hand" onClick={sort('password')}>
                  <Translate contentKey="ivyModaApp.userAccount.password">Password</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('password')} />
                </th>
                <th className="hand" onClick={sort('address')}>
                  <Translate contentKey="ivyModaApp.userAccount.address">Address</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('address')} />
                </th>
                <th className="hand" onClick={sort('isActive')}>
                  <Translate contentKey="ivyModaApp.userAccount.isActive">Is Active</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('isActive')} />
                </th>
                <th>
                  <Translate contentKey="ivyModaApp.userAccount.role">Role</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {userAccountList.map((userAccount, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{userAccount.role ? <Link to={`/role/${userAccount.role.id}`}>{userAccount.role.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="ivyModaApp.userAccount.home.notFound">No User Accounts found</Translate>
            </div>
          )
        )}
      </div>
