  ...
  return (
    <div>
      <h2 id="cart-heading" data-cy="CartHeading">
        <Translate contentKey="ivyModaApp.cart.home.title">Carts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="ivyModaApp.cart.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/cart/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="ivyModaApp.cart.home.createLabel">Create new Cart</Translate>
          </Link>
        </div>
      </h2>
        ...
        {cartList && cartList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="ivyModaApp.cart.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('createAt')}>
                  <Translate contentKey="ivyModaApp.cart.createAt">Create At</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('createAt')} />
                </th>
                <th>
                  <Translate contentKey="ivyModaApp.cart.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {cartList.map((cart, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{cart.createAt ? <TextFormat type="date" value={cart.createAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{cart.user ? <Link to={`/user-account/${cart.user.id}`}>{cart.user.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="ivyModaApp.cart.home.notFound">No Carts found</Translate>
            </div>
          )
        )}
      </div>
